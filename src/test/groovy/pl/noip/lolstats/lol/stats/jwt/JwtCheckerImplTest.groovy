...
class JwtCheckerImplTest extends Specification {
    ...
    def "check valid token"() {
        ...
        def token = jwtGenerator.generate(new Account(mail, "dasga", null, null))
    ...
    def "check expired token"() {
        ...
        and: "new token"
        def mail = "example@mail.com"
        def token = jwtGenerator.generate(new Account(mail, "fdasga", null, null))
        when: "token is checked"
        jwtChecker.checkToken(token)
    ...
    def "check token with incorrect signature"() {
        ...
        and: "new token"
        def mail = "example@mail.com"
        def token = jwtGenerator.generate(new Account(mail, "gsrfsds", null, null))
